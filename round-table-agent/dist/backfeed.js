...
function classifyDim(description) {
    for (const d of DIM_KEYWORDS) {
        if (d.keywords.some((k) => description.includes(k)))
            return d.dim;
    }
    return "D2 用词独特性";
}
function toM1Row(item, idx) {
...
export function generateDraft(items, topic) {
    ...
    lines.push(``);
    constraints.forEach((b) => lines.push(`- **M3-禁区（待编号）**：${b.description}（来源 ${b.id}）`));
    lines.push(``);
    lines.push(`## C. 误判防护提示（来自"反例"边界，共 ${counterExamples.length} 条）`);
    lines.push(``);
    counterExamples.forEach((b) => lines.push(`- **判定注意**：${b.description}（来源 ${b.id}）`));
    lines.push(``);
...
async function main() {
    ...
    let snapshot = null;
    const candidate = arg.includes(".json") || arg.includes("\\") || arg.includes("/") ? arg : join(MEETINGS_DIR, `${arg}-snapshot.json`);
    try {
