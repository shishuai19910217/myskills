...
function pushEvent(rt, e) {
    rt.events.push(e);
    for (const res of rt.subscribers) {
        res.write(`data: ${JSON.stringify(e)}\n\n`);
    }
}
function startMeeting(input) {
...
app.post("/api/meetings", (req, res) => {
    ...
    const input = { id: randomUUID(), source, content, meta };
...
app.post("/api/meetings/:id/turn", async (req, res) => {
    ...
    const turn = kind === "new_boundary" || kind === "supplement" || kind === "stop"
        ? { kind, ...(kind === "new_boundary" ? { item: { type: "目标", description: content, confidence: "高" } } : kind === "supplement" ? { supplement: content } : {}) }
...
// 当前清单
app.get("/api/meetings/:id/list", (req, res) => {
    const rt = meetings.get(req.params.id);
    if (!rt) {
        res.status(404).json({ error: "会议不存在" });
        return;
