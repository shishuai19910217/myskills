import type { BoundaryItem, BoundaryType, SeatRole } from "./types.js";
...
export const SEAT_BOUNDARY_TYPE: Record<Exclude<SeatRole, "coordinator">, BoundaryType> = {
  forward: "目标",
  reverse: "约束",
  counter: "反例",
  assumption: "前提",
...
export interface SeatSystemParams {
  topic: string;
  boundaryType: BoundaryType;
  label: string;
}

export function seatSystemPrompt({ label, boundaryType, topic }: SeatSystemParams): string {
  const typeHint =
    boundaryType === "目标"
...
export function coordinatorOpenUser(input: string): string {
...
export function coordinatorConvergeUser(params: {
  topic: string;
  pool: BoundaryItem[];
  recentRounds: { round: number; newCount: number }[];
}): string {
  const poolText =
    params.pool.length === 0
      ? "（空）"
