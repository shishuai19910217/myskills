...
import type { BoundaryList, MeetingSnapshot } from "./types.js";
...
export function persistSnapshot(meetingId: string, snapshot: MeetingSnapshot): string {
  ensureDir();
...
export function persistReport(meetingId: string, list: BoundaryList, solution: string): string {
  ensureDir();
  const file = join(MEETINGS_DIR, `${meetingId}-report.md`);
  const itemsText = list.items
    .map(
      (b) =>
        `- ${b.id} [${b.type}] ${b.description}（${b.proposer}，可信度:${b.confidence}` +
