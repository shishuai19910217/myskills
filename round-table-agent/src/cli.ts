...
import type { BoundaryItem, MeetingInput } from "./types.js";

function printList(list: { topic: string; objective: string; scope: string; items: BoundaryItem[]; rounds: number; convergence: string; disputesResolved: number; tokenEstimate: number }) {
  console.log("\n════════ 边界约束清单 ════════");
...
async function main() {
  ...
  const input: MeetingInput = { id: `meeting-${Date.now()}`, source: "text", content: inputText };
