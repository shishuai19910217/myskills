...
async function probe(id: string, baseURL: string, model: string): Promise<string> {
  try {
...
async function main() {
  ...
  if (ok.length < 2) {
    ...
    const normalized = ok.map(([k, r]) => [k, r.slice(0, 60)] as const);
    for (let i = 0; i < normalized.length; i++) {
      for (let j = i + 1; j < normalized.length; j++) {
        const [a, ra] = normalized[i];
        const [b, rb] = normalized[j];
        if (ra === rb) console.log(`⚠ ${a} 与 ${b} 输出完全相同 → 疑似同源`);
        else console.log(`✓ ${a} 与 ${b} 输出不同 → 基本判定非同源`);
      }
    }
  }
